...
interface ITrustRegistryV3 {
    ...
    /// @param resolvedAt       Timestamp when the agreement was resolved; used for recency weighting
    ...
    /// @dev B-6: access-controlled — only authorised updaters (added via addUpdater()) may call this.
    ///      Any caller not in the isAuthorizedUpdater mapping will be rejected.
    /// @param wallet The wallet to initialise
    function initWallet(address wallet) external;

    /// @notice Apply a vouching boost to a wallet's trust score. Called by VouchingRegistry on vouch.
    /// @param wallet The wallet to boost.
    /// @param boost Points to add (capped at MAX_SCORE).
    function applyBoost(address wallet, uint256 boost) external;

    /// @notice Reverse a vouching boost previously applied to a wallet. Called by VouchingRegistry on clean revoke.
