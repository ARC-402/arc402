...
contract SessionChannels is ISessionChannels, ReentrancyGuard {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "SessionChannels: not owner");
        _;
    }
    ...
    function _updateChannelTrust(bytes32, ChannelTypes.Channel storage ch, bool providerSuccess, bool clientSuccess) internal {
        ...
        if (providerSuccess) {
            try ITrustRegistry(tr).recordSuccess(ch.provider, ch.client, "session-channel", ch.settledAmount, block.timestamp) {} catch {}
        }
        if (clientSuccess) {
            try ITrustRegistry(tr).recordSuccess(ch.client, ch.provider, "session-channel", ch.settledAmount, block.timestamp) {} catch {}
        }
    }

    // ─── EIP-712 Domain Separation (MA-07) ───────────────────────────────────

    bytes32 private constant DOMAIN_TYPEHASH =
        keccak256("EIP712Domain(string name,uint256 chainId,address verifyingContract)");

    bytes32 private constant CHANNEL_STATE_TYPEHASH =
        keccak256("ChannelState(bytes32 channelId,uint256 sequenceNumber,uint256 callCount,uint256 cumulativePayment,address token,uint256 timestamp)");

    /// @dev MA-07: Domain separator includes chainId and verifying contract address to prevent
    ///      cross-chain and cross-contract signature replay attacks.
    function _domainSeparator() internal view returns (bytes32) {
        return keccak256(abi.encode(
            DOMAIN_TYPEHASH,
            keccak256("ARC402SessionChannels"),
            block.chainid,
            address(this)
        ));
    }
    ...
    function _verifyChannelStateSigs(ChannelTypes.Channel storage ch, ChannelTypes.ChannelState memory state) internal view {
        ...
        address clientSigner = ECDSA.recover(digest, state.clientSig);
        address providerSigner = ECDSA.recover(digest, state.providerSig);
