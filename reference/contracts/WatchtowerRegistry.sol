 ...
 *
 * MA-01 FIX: _requireChannelClient now queries SessionChannels directly for channel
 * client lookup. ServiceAgreement does not expose a `channels()` public mapping —
 * channel state lives in SessionChannels. The public `channels(bytes32)` getter on
 * SessionChannels returns all fields of ChannelTypes.Channel.
 */

/// @dev Interface for querying channel client from SessionChannels.
///      SessionChannels has `mapping(bytes32 => ChannelTypes.Channel) public channels`
///      whose auto-generated getter matches this signature.
interface ISessionChannelsForWatchtower {
...
/// @dev Minimal interface for submitting challenges through ServiceAgreement.
...
contract WatchtowerRegistry is IWatchtowerRegistry {
    ...
    /// @dev MA-12: Deduplication set — prevents the same channelId being pushed
    ///      to watchedChannels[watchtower] more than once on re-authorization.
    mapping(address => mapping(bytes32 => bool)) private _watchedChannelSet;

    address public owner;
    IServiceAgreementForWatchtower public serviceAgreement;
    ISessionChannelsForWatchtower public sessionChannels;

    // ─── Admin events ────────────────────────────────────────────────────────

    event ServiceAgreementUpdated(address indexed oldServiceAgreement, address indexed newServiceAgreement);
    event SessionChannelsUpdated(address indexed oldSessionChannels, address indexed newSessionChannels);

    // ─── Modifiers ───────────────────────────────────────────────────────────

    modifier onlyOwner() {
        require(msg.sender == owner, "WatchtowerRegistry: not owner");
        _;
    }

    // ─── Constructor ─────────────────────────────────────────────────────────

    constructor(address _serviceAgreement, address _sessionChannels) {
        require(_serviceAgreement != address(0), "WatchtowerRegistry: zero service agreement");
        require(_sessionChannels != address(0), "WatchtowerRegistry: zero session channels");
        owner = msg.sender;
        serviceAgreement = IServiceAgreementForWatchtower(_serviceAgreement);
        sessionChannels = ISessionChannelsForWatchtower(_sessionChannels);
    }

    function setServiceAgreement(address _serviceAgreement) external onlyOwner {
        require(_serviceAgreement != address(0), "WatchtowerRegistry: zero address");
        emit ServiceAgreementUpdated(address(serviceAgreement), _serviceAgreement);
        serviceAgreement = IServiceAgreementForWatchtower(_serviceAgreement);
    }

    function setSessionChannels(address _sessionChannels) external onlyOwner {
        require(_sessionChannels != address(0), "WatchtowerRegistry: zero address");
        emit SessionChannelsUpdated(address(sessionChannels), _sessionChannels);
        sessionChannels = ISessionChannelsForWatchtower(_sessionChannels);
    }
    ...
    function authorizeWatchtower(bytes32 channelId, address watchtower) external {
        ...
        // MA-12: Only push if not already tracked — prevents duplicate entries on re-authorization.
        if (!_watchedChannelSet[watchtower][channelId]) {
            _watchedChannelSet[watchtower][channelId] = true;
    ...
    ///      Queries SessionChannels directly — SA has no public channels() mapping.
    function _requireChannelClient(bytes32 channelId) internal view {
        (address client, , , , , , , , ) = sessionChannels.channels(channelId);
