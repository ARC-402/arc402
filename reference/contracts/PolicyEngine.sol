...
contract PolicyEngine is IPolicyEngine {
    ...
    // F-01 / Attacker-INFO-3: contextId deduplication scoped per wallet+contextId.
    // Previously a global mapping(bytes32 => bool) — the first wallet to spend in any context
    // would permanently block the same contextId across ALL wallets (cross-wallet griefing).
    // Now keyed by (wallet, contextId) so each wallet tracks its own closed contexts.
    // A contextId is only marked used when the wallet calls closeContext() — NOT on first spend —
    // so multiple spends within the same open context are permitted (F-01 fix).
    mapping(address => mapping(bytes32 => bool)) private _usedContextIds;
    ...
    function validateSpend(
        ...
        // F-01: contextId replay check — only rejects a *closed* context (not an active one).
        // _usedContextIds is set by closeContext(), not by recordSpend().
        // Multiple spends within the same open context are allowed.
        if (contextId != bytes32(0) && _usedContextIds[wallet][contextId]) {
    ...
    function recordSpend(
        ...
        // F-01: Do NOT mark contextId as used here. Multiple spends per context are valid.
        // The contextId is marked used when the wallet closes its context via closeContext().

        emit SpendRecorded(wallet, category, amount, contextId);
    }

    /// @notice Mark a context as closed so it cannot be reopened.
    ///         Called by ARC402Wallet.closeContext() after the context window ends.
    ///         Only callable by the wallet itself or its registered owner.
    /// @dev F-01: This is where contextId uniqueness is enforced — NOT in recordSpend.
    ///      Multiple spends within the same context are allowed; reopening a closed
    ///      context ID is not. contextId == 0 is a no-op (0 is the "no context" sentinel).
    function closeContext(address wallet, bytes32 contextId) external {
        require(msg.sender == wallet || msg.sender == walletOwners[wallet], "PolicyEngine: not authorized");
        if (contextId != bytes32(0)) {
            _usedContextIds[wallet][contextId] = true;
        }
    }
