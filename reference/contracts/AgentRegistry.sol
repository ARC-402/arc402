...
/**
 * @dev Minimal interface for reading an ARC402Wallet's owner EOA.
 *      All ARC402Wallet contracts extend Ownable, so owner() is always available.
 */
...
contract AgentRegistry is IAgentRegistry, Ownable2Step {
    ...
    /// @notice Optional MigrationRegistry for lineage-ownership enforcement on registration.
    ///         When set, register() verifies that msg.sender's migration lineage owner matches
    ///         the current owner of the lineage root wallet, blocking sold-key re-registration.
    ...
    /// @notice Set the MigrationRegistry address for lineage-ownership checks on registration.
    ///         Pass address(0) to disable lineage enforcement.
    function setMigrationRegistry(address registry) external onlyOwner {
        migrationRegistry = registry;
        emit MigrationRegistryUpdated(registry);
    }
    ...
    function register(
        ...
        // ── Migration lineage ownership check ───────────────────────────────
        // If this wallet appears in a migration lineage (i.e. it was migrated TO from an older
        // wallet), verify that the current Ownable owner of the lineage root matches the current
        // Ownable owner of msg.sender. This prevents a buyer of a sold key from re-registering
        // the wallet under a different ownership identity than the original lineage owner.
        if (migrationRegistry != address(0)) {
            try IMigrationRegistry(migrationRegistry).getLineage(msg.sender) returns (address[] memory lineage) {
                if (lineage.length > 1) {
                    address lineageRoot = lineage[0];
                    address rootOwner   = IWalletOwnable(lineageRoot).owner();
                    address senderOwner = IWalletOwnable(msg.sender).owner();
                    require(
                        rootOwner == senderOwner,
                        "AgentRegistry: migration lineage owner mismatch"
                    );
                }
            } catch {} // solhint-disable-line no-empty-blocks — skip check if registry call fails
        }
