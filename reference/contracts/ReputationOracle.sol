...
contract ReputationOracle {
    ...
    event TrustRegistryUpdated(address indexed oldTrustRegistry, address indexed newTrustRegistry);
    ...
    function setTrustRegistry(address _trustRegistry) external onlyOwner {
        require(_trustRegistry != address(0), "ReputationOracle: zero address");
        emit TrustRegistryUpdated(address(trustRegistry), _trustRegistry);
        trustRegistry = ITrustRegistry(_trustRegistry);
    }

    function setServiceAgreement(address _serviceAgreement) external onlyOwner {
        require(_serviceAgreement != address(0), "ReputationOracle: zero address");
        emit ServiceAgreementUpdated(serviceAgreement, _serviceAgreement);
        serviceAgreement = _serviceAgreement;
    }
