...
contract ServiceAgreementTrack1Test is Test {
    ...
    function setUp() public {
        ...
        dm = new DisputeModule(_saAddr);
        sa = new ServiceAgreement(address(registry), address(dm), address(0));
...
contract ARC402WalletRegistryPendingTest is Test {
    ...
    function setUp() public {
        ...
        TrustRegistryV3 tr = new TrustRegistryV3(address(0));
        IntentAttestation ia = new IntentAttestation();
        SettlementCoordinator sc = new SettlementCoordinator();
        reg = new ARC402RegistryV2(address(pe), address(tr), address(ia), address(sc), "v1");
        // B-6: addUpdater before deploying wallet — wallet constructor calls initWallet
        tr.addUpdater(address(this));
        wallet = new ARC402Wallet(address(reg), address(this), address(0xE4337));
    }

    function _deployReg(string memory version) internal returns (ARC402RegistryV2) {
        PolicyEngine pe = new PolicyEngine();
        TrustRegistryV3 tr = new TrustRegistryV3(address(0));
        IntentAttestation ia = new IntentAttestation();
        SettlementCoordinator sc = new SettlementCoordinator();
        return new ARC402RegistryV2(address(pe), address(tr), address(ia), address(sc), version);
    }

    function test_RegistryTimelock_RejectsReproposalWhilePending() public {
        ARC402RegistryV2 reg2 = _deployReg("v2");
        ARC402RegistryV2 reg3 = _deployReg("v3");
        wallet.proposeRegistryUpdate(address(reg2));
        vm.expectRevert(WPending.selector);
