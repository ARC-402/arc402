...
contract BrokenTrustRegistry is ITrustRegistry {
    ...
    function recordSuccess(address, address, string calldata, uint256, uint256) external pure override {
...
contract ServiceAgreementTrustLivenessTest is Test {
    ...
    function setUp() public {
        ...
        address _saAddr = vm.computeCreateAddress(address(this), _nonce + 1);
        dm = new DisputeModule(_saAddr);
        sa = new ServiceAgreement(address(brokenRegistry), address(dm), address(0));
        sa.setLegacyFulfillMode(true);
        sa.setLegacyFulfillProvider(provider, true);
