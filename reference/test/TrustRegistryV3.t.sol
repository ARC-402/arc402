 ...
 * @notice Foundry test suite for TrustRegistryV3 — capability-specific Sybil-resistant trust.
 *
 * Coverage:
...
contract TrustRegistryV2Test is Test {
    ...
    function test_InitWallet_Idempotent() public {
        registry.initWallet(WALLET);
        registry.recordSuccess(WALLET, COUNTERPARTY, "compute", REF_VALUE, block.timestamp);
    ...
    function test_RecordSuccess_ValueWeighted() public {
        ...
        registry.recordSuccess(walletA, COUNTERPARTY, "compute", REF_VALUE, block.timestamp);  // 0.01 ETH → +5
        registry.recordSuccess(walletB, COUNTERPARTY, "compute", 1 ether, block.timestamp);    // 1 ETH → +25
    ...
    function test_RecordSuccess_ValueCap() public {
        registry.initWallet(WALLET);
        registry.recordSuccess(WALLET, COUNTERPARTY, "compute", 10 ether, block.timestamp);
    ...
    function test_RecordSuccess_DifferentCounterparties_FullGain() public {
        registry.initWallet(WALLET);
        registry.recordSuccess(WALLET, address(0xC1), "compute", REF_VALUE, block.timestamp); // gain=5
        vm.roll(block.number + 1);
        registry.recordSuccess(WALLET, address(0xC2), "compute", REF_VALUE, block.timestamp); // gain=5 (new CP)
    ...
    function test_EffectiveScore_TimeDecay() public {
        ...
        for (uint256 i = 0; i < 32; i++) {
            ...
            registry.recordSuccess(WALLET, address(uint160(0xC000 + i)), "compute", 1 ether, block.timestamp);
            vm.roll(block.number + 1);
        }
    ...
    function test_EffectiveScore_FullDecay() public {
        ...
        // Build a high score
        for (uint256 i = 0; i < 32; i++) {
            registry.recordSuccess(WALLET, address(uint160(0xD000 + i)), "compute", 1 ether, block.timestamp);
            vm.roll(block.number + 1);
        }
    ...
    function test_EffectiveScore_NoDecayImmediately() public {
        registry.initWallet(WALLET);
        registry.recordSuccess(WALLET, COUNTERPARTY, "compute", 1 ether, block.timestamp);
    ...
    function test_CapabilityScore_TopFive() public {
        ...
        for (uint256 i = 0; i < 6; i++) {
            registry.recordSuccess(WALLET, COUNTERPARTY, caps[i], REF_VALUE, block.timestamp);
            vm.roll(block.number + 1);
        }
    ...
    function test_CapabilityScore_HigherScoreDisplacesLowest() public {
        ...
        for (uint256 i = 0; i < 5; i++) {
            registry.recordSuccess(WALLET, COUNTERPARTY, base[i], REF_VALUE, block.timestamp);
            vm.roll(block.number + 1);
        }

        // Now record a high-value agreement for a 6th capability → score 125 (>105)
        registry.recordSuccess(WALLET, COUNTERPARTY, "highval", 1 ether, block.timestamp);
    ...
    function test_MinimumAgreementValue_SkipsSmall() public {
        ...
        registry.recordSuccess(WALLET, COUNTERPARTY, "compute", 1, block.timestamp);
    ...
    function test_MinimumAgreementValue_ExactMinimumProcessed() public {
        ...
        registry.recordSuccess(WALLET, COUNTERPARTY, "compute", REF_VALUE, block.timestamp); // exactly at min
    ...
    function test_MinimumAgreementValue_ZeroDisabled() public {
        ...
        registry.recordSuccess(WALLET, COUNTERPARTY, "compute", 1, block.timestamp); // 1 wei, gate disabled
    ...
    function test_V1Migration() public {
        ...
        for (uint256 i = 0; i < 80; i++) {
            v1.recordSuccess(WALLET, address(uint160(i + 1)), "legacy", REF_VALUE, block.timestamp);
        }
        assertEq(v1.getScore(WALLET), 500, "v1 score should be 500");

        // Deploy v2 pointing at v1
        TrustRegistryV3 v2 = new TrustRegistryV3(address(v1));
    ...
    function test_Auth_RecordSuccess_Unauthorised() public {
        ...
        vm.expectRevert("TrustRegistryV3: not authorized updater");
        registry.recordSuccess(WALLET, COUNTERPARTY, "compute", REF_VALUE, block.timestamp);
    }
    ...
    function test_MeetsThreshold_True() public {
        registry.initWallet(WALLET);
        registry.recordSuccess(WALLET, COUNTERPARTY, "compute", 1 ether, block.timestamp); // → 125
    ...
    function test_MeetsCapabilityThreshold() public {
        registry.initWallet(WALLET);
        registry.recordSuccess(WALLET, COUNTERPARTY, "compute", 1 ether, block.timestamp); // cap score = 125
