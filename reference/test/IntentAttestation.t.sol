...
contract IntentAttestationTest is Test {
    ...
    function test_attest_and_verify() public {
        attestor.attest(ID, "pay_provider", "Test payment", address(0x999), 0.1 ether);
    ...
    function test_immutability_cannotReattest() public {
        ...
        attestor.attest(ID, "pay_provider_2", "Different", address(0x999), 0.2 ether);
    }

    function test_getAttestation() public {
        attestor.attest(ID, "acquire_records", "Medical records for claim", address(0x999), 0.05 ether);
        (bytes32 id, address wallet, string memory action, string memory reason, address recipient, uint256 amount,) = attestor.getAttestation(ID);
        assertEq(id, ID);
