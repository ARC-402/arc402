...
contract ServiceAgreementChannelTest is Test {
    ...
    function setUp() public {
        ...
        address _saAddr = vm.computeCreateAddress(address(this), _nonce + 2);
        DisputeModule dm = new DisputeModule(_saAddr);
        sc = new SessionChannels(_saAddr);
        sa = new ServiceAgreement(address(trustReg), address(dm), address(sc));
    ...
    bytes32 private constant _CHANNEL_STATE_TYPEHASH =
    ...
    function _testDomainSeparator() internal view returns (bytes32) {
    ...
    function _signState(
        ...
        bytes32 structHash = keccak256(abi.encode(
            _CHANNEL_STATE_TYPEHASH,
            chId, seq, callCount, cumPayment, tok, ts
        ));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", _testDomainSeparator(), structHash));
