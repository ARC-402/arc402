...
abstract contract AuditBaseTest is Test {
    ...
    function _baseSetUp() internal {
        ...
        address _saAddr = vm.computeCreateAddress(address(this), _nonce + 1);
        DisputeModule dm = new DisputeModule(_saAddr);
        sa = new ServiceAgreement(address(trustReg), address(dm), address(0));
...
contract AuditFix_B03_PartialSettlementTerminal is AuditBaseTest {
    ...
    function setUp() public {
        _baseSetUp();
        da = new DisputeArbitration(address(trustReg), treasury, address(sa), sa.disputeModule());
...
contract AuditFix_B07_EffectiveScoreArbitratorCheck is Test {
    ...
    function setUp() public {
        trustRegV2 = new TrustRegistryV3(address(0)); // no v1 migration
        da = new DisputeArbitration(address(trustRegV2), treasury, saAddr, address(0));
        da.setTokenUsdRate(address(0), 2000e18);
    ...
    function test_B07_EffectiveScoreUsedNotRawScore() public {
        ...
        // TrustRegistryV3.getEffectiveScore applies time decay toward floor.
...
contract AuditFix_R02_NoCatchOnRecordVote is Test {
    ...
    function setUp() public {
        ...
        da = new DisputeArbitration(address(trustReg), treasury, address(sa), address(dm));
        da.setTokenUsdRate(address(0), 2000e18);
        trustReg.addUpdater(address(da));
        sa.setDisputeArbitration(address(da));
...
contract AuditFix_R03_Ownable2Step is Test {
    ...
    function setUp() public {
        trustReg = new TrustRegistry();
        da = new DisputeArbitration(address(trustReg), treasury, address(0), address(0));
    }
...
contract AuditFix_R05_BondRecovery is Test {
    ...
    function setUp() public {
        trustReg = new TrustRegistry();
        da = new DisputeArbitration(address(trustReg), treasury, saAddr, address(0));
        da.setTokenUsdRate(address(0), 2000e18);
        trustReg.addUpdater(address(da));
        trustReg.initWallet(arb1);
