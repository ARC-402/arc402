...
export class ChannelClient {
  ...
  static domainSeparator(chainId: bigint, contractAddress: string): string {
    ...
    return ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(
      ...
      [DOMAIN_TYPEHASH, ethers.keccak256(ethers.toUtf8Bytes("ARC402SessionChannels")), chainId, contractAddress]
    ));
  }
   ...
   */
  static channelStateDigest(
    state: Omit<ChannelState, "clientSig" | "providerSig">,
    chainId: bigint,
    contractAddress: string
  ): Uint8Array {
    const CHANNEL_STATE_TYPEHASH = ethers.keccak256(
      ethers.toUtf8Bytes("ChannelState(bytes32 channelId,uint256 sequenceNumber,uint256 callCount,uint256 cumulativePayment,address token,uint256 timestamp)")
    );
    const structHash = ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(
      ...
      [CHANNEL_STATE_TYPEHASH, state.channelId, state.sequenceNumber, state.callCount, state.cumulativePayment, state.token, state.timestamp]
    ));
    const domainSep = ChannelClient.domainSeparator(chainId, contractAddress);
    return ethers.getBytes(ethers.keccak256(
      ethers.concat([new Uint8Array([0x19, 0x01]), ethers.getBytes(domainSep), ethers.getBytes(structHash)])
    ));
  }
  ...
  async signStateUpdate(
    ...
    const sig = await this.signer!.signMessage(ethers.getBytes(digest));
    return {
      ...stateData,
  ...
  async verifyStateUpdate(state: ChannelState, channel: Channel, chainId: bigint, contractAddress: string): Promise<boolean> {
    const stateData = {
    ...
    try {
      if (state.clientSig) {
        const clientSigner = ethers.verifyMessage(digest, state.clientSig);
        if (clientSigner.toLowerCase() !== channel.client.toLowerCase()) return false;
      }
      if (state.providerSig) {
        const providerSigner = ethers.verifyMessage(digest, state.providerSig);
