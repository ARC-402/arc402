...
export function registerWorkroomCommands(program: Command): void {
  ...
  workroom
    ...
    .action(async () => {
      ...
      // before iptables is applied (the entrypoint resolves these IPs at startup).
      // Without this, the daemon can't connect to the chain and the workroom is dead on arrival.
      if (!fs.existsSync(POLICY_FILE)) {
        console.log(c.dim("No policy file found. Generating bootstrap policy..."));
        const bootstrapPolicy = `# ARC-402 Workroom Policy (auto-generated by workroom init)
# Edit this file to control which hosts the workroom can reach.
# Run 'arc402 workroom policy preset core-launch' to apply a full preset.
...
network_policies:
  ...
  arc402_infra:
    name: arc402-infrastructure
    endpoints:
      - host: api.arc402.xyz
        port: 443
      - host: gigabrain.arc402.xyz
        port: 443
      - host: megabrain.arc402.xyz
        port: 443
...
`;
        ...
        fs.writeFileSync(POLICY_FILE, bootstrapPolicy, "utf-8");
        console.log(" " + c.success + c.white(" Bootstrap policy written (Base RPC + bundler + ARC-402 infra + LLM APIs)"));
        console.log(c.dim("   Run 'arc402 workroom policy preset core-launch' to expand the policy."));
      } else {
  ...
  workroom
    ...
    .action(async (host) => {
      ...
      if (result.ok && result.stdout.trim() !== "000") {
        ...
        console.log(c.dim("  Add it through the workroom policy flow."));
      }
    });
